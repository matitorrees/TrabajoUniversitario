...
{
    public partial class ModificarInquilino : UserControl
    {
        Inquilino p;
        public event ModifInquilino ModInquilino;
        public event GuardarInquilino ActualizarControlInquilino;
        ...
        public void CargarTextBox(Inquilino p1)
        {
            p = p1;
        ...
        private void BtnGuardar_Click(object sender, EventArgs e) //Guardar
        {
            ...
            try
            {
                ...
                Inquilino propietarioNuevo = new Inquilino(p.NroPropietario, textBoxNombreProf.Text, dni, tel, cel, mail, richTextBoxComProf.Text);
               // propietarioNuevo.Propiedades = p.Propiedades;

                if (ModInquilino(propietarioNuevo, p))
                {
                    Sacatelo(p.NroPropietario.ToString());
                    ActualizarControlInquilino(propietarioNuevo);
                    this.textBoxCelProf.Clear();
                    this.textBoxDniProf.Clear();
                    this.textBoxMailProf.Clear();
                    this.textBoxNombreProf.Clear();
